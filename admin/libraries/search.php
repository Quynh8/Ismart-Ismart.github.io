...
function db_search_all_admins($value){
    $sql = "SELECT * FROM `tbl_users` WHERE  CONVERT(`username` USING utf8) LIKE '%$value %' OR CONVERT(`fullname` USING utf8) LIKE '%$value %'";
    $result = db_fetch_array($sql);
    return $result;
}
# search admins by page
function db_search_admins_by_page($value, $start = 1, $num_per_page = 10){
    $sql = "SELECT * FROM `tbl_users` WHERE  CONVERT(`username` USING utf8) LIKE '%$value %' OR CONVERT(`fullname` USING utf8) LIKE '%$value %' LIMIT {$start}, {$num_per_page}";
    $result = db_fetch_array($sql);
    return $result;
}
// search  all pages
...
function db_search_all_medias($value){
    ...
    ."SELECT * FROM `tbl_users` WHERE CONVERT(`avatar` USING utf8) LIKE '%$value %'"
