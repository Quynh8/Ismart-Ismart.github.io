...
<div id="main-content-wp" class="list-post-page">
                    ...
                    <form method="POST" action="" class="form-actions">
                        ...
                        <table class="table list-table-wp">
                           ...
                           <?php if(!empty($list_product_cat)){
                            ...
                            <tbody>
                                <tr>
                                    <td><input type="checkbox" name="checkItem[]" class="checkItem" value="<?php echo $item['product_cat_id'] ?>"></td>
                                    <td><span class="tbody-text"><?php echo $temp; ?></h3></span>
                                    <td class="clearfix">
                                        <div class="tb-title fl-left">
                                            <a href="?mod=product&controller=product_cat&action=edit_cat&product_cat_id=<?php echo $item['product_cat_id'] ?>" title=""><?php echo $item['product_cat_title'] ?></a>
                                        </div>
                                        <ul class="list-operation fl-right">
                                            <li><a href="?mod=product&controller=product_cat&action=edit_cat&product_cat_id=<?php echo $item['product_cat_id'] ?>" title="Sửa" class="edit"><i class="fa fa-pencil" aria-hidden="true"></i></a></li>
                                            <li><a href="?mod=product&controller=product_cat&action=delete_cat&product_cat_id=<?php echo $item['product_cat_id'] ?>" title="Xóa" class="delete"><i class="fa fa-trash" aria-hidden="true"></i></a></li>
                                        </ul>
                                    </td>
                                   
                                    <td><span class="tbody-text"><?php echo show_status("tbl_product_cat","product_cat_id",$item['product_cat_id']); ?></span></td>
   ...
   //Cần truyền vào hàm tổng số trang,trang hiện tại và đường dẫn mặc định
    echo get_pagging($num_page,$page,"?mod=product&controller=product_cat&action=index")
   ?>
                </div>
            </div>
        </div>
